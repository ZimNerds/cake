...
<div class="usersRecords index large-9 medium-8 columns content">
    <h3><?= __('Users Records') ?></h3>
    <table cellpadding="0" cellspacing="0">
        ...
        <tbody>
            <?php foreach ($usersRecords as $usersRecord): ?>
            <tr>
                ...
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $usersRecord->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $usersRecord->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $usersRecord->id], ['confirm' => __('Are you sure you want to delete # {0}?', $usersRecord->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
